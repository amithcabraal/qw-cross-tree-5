...
const Board: React.FC<BoardProps> = ({ 
  ...
  const getHighlightCells = (row: number, col: number): string => {
    if (!board[row][col]) return '';
    
    let classes = [];
    
    // Highlight same row
    if (board[row].some(cell => cell === 'tree')) {
      classes.push('row-highlight');
    }
    
    // Highlight same column
    if (board.some(r => r[col] === 'tree')) {
      classes.push('col-highlight');
    }
    ...
    const hasAdjacentTree = adjacentCells.some(([dx, dy]) => {
      const newRow = row + dx;
      const newCol = col + dy;
      return (
    ...
    if (hasAdjacentTree) {
      classes.push('adjacent-highlight');
    }
